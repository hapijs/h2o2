...
internals.handler = function (route, handlerOptions) {
    ...
    var modifyPayload = handlerOptions.modifyPayload || false;
    var modifyResponse = handlerOptions.modifyResponse || false;
    ...
    return function (request, reply) {
        ...
        settings.mapUri(request, function (err, uri, headers) {
            ...
            var _sendResponse = function (res, ttl, settings, data) {
                var response = reply(data.body ? data.body : res)
                .header('content-type', 'application/json') // sindicetech hack should be set only when there is a data.body
                .ttl(ttl)
                .code(res.statusCode)
                ...
                Wreck.request(request.method, uri, options, function (err, res) {
                    ...
                    if (modifyResponse && data) {
                        modifyResponse(res, data).then(function (ret) {
                            _sendResponse(ret.response, ttl, settings, ret.data);
                        });
                    } else {
                        _sendResponse(res, ttl, settings);
                    }
                });
            };
