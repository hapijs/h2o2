...
internals.handler = function (route, handlerOptions) {
    ...
    Hoek.assert(
        ...
        ((route.settings.payload.output === 'data' || route.settings.payload.output === 'stream') && !route.settings.payload.parse),
    ...
    return async function (request, h) {
        ...
        try {
            const { uri, headers } = settings.mapUri(request);
            ...
            if (settings.passThrough) {
                ...
                if (options.headers.cookie) { //TODO: Add documentation for server.states.passThrough
